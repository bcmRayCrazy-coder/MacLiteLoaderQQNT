...
const admZip = (() => {
    const major_node = path.join(resourcesPath, 'app', 'major.node');
...
function InstallPlugin(slug) {
    try {
        ...
        if (fs.existsSync(dest_path)) {
            fs.renameSync(
                dest_path,
                `${dest_path}_${parseInt(Math.random() * 100000)} `
            );
        }
        if (plugin_type == 'zip') {
        ...
        if (plugin_type == 'json') {
            fs.cpSync(path.dirname(plugin_path), dest_path, {
                recursive: true,
            });
        }
    } catch (error) {
...
function findAllPlugin() {
    ...
    try {
        ...
        for (const pathname of fs.readdirSync(
...
function getPluginInfo(pathname, manifest) {
    const incompatible_platform = !manifest.platform.includes(
    ...
    const preload_file = path.join(
        plugin_path,
        manifest?.injects?.preload ?? ''
    );
    const renderer_file = path.join(
        plugin_path,
        manifest?.injects?.renderer ?? ''
    );
    return {
        manifest: manifest,
        ...
        path: {
            ...
            injects: {
                ...
                preload: fs.statSync(preload_file).isFile()
                    ? preload_file
                    : null,
                renderer: fs.statSync(renderer_file).isFile()
                    ? renderer_file
                    : null,
            },
        },
