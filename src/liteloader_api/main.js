...
const qwqnt_path = path.join(
    globalThis.qwqnt?.framework?.paths?.data ?? '',
    'LiteLoader'
);
const root_path = path.join(__dirname, '..', '..');
const profile_path =
    process.env.LITELOADERQQNT_PROFILE ??
    (globalThis.qwqnt ? qwqnt_path : root_path);
...
const qqnt_package = require(path.join(resourcesPath, 'app/package.json'));
            ...
            if (path.extname(plugin_path).toLowerCase() == '.zip') {
                ...
                for (const entry of plugin_zip.getEntries()) {
                    if (
                        ...
                        if (slug in LiteLoader.plugins)
                            LiteLoader.api.plugin.delete(slug, false, false);
                        const config = LiteLoader.api.config.get(
                            'LiteLoader',
                            default_config
                        );
                        if (undone) delete config.installing_plugins[slug];
                        else
                            config.installing_plugins[slug] = {
